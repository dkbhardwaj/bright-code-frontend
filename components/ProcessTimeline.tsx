                        ...
                        return (
                            <div key={index} className="flex w-full">
                                ...
                                <div className="relative flex flex-col items-center mr-6 md:mr-12 shrink-0">
                                    {/* The Circle */}
                                    <div className="flex items-center justify-center w-[32px] h-[32px] rounded-full border border-[#e2e8f0] bg-white text-black text-[12px] font-medium z-10 relative mt-2">
                                        {index + 1}
                                    </div>
                                    {/* The Connecting Line */}
                                        <div className="absolute top-[32px] bottom-[-32px] md:bottom-[-48px] left-1/2 -translate-x-1/2 w-px bg-[#e2e8f0]" />
                                </div>

                                {/* Right Column: Content */}
                                <div className={`flex-1 pb-8 md:pb-12 border-b border-[#e2e8f0] ${!isLast ? 'mb-8 md:mb-12' : ''}`}>
                                    <h4 className="text-[#64748b] uppercase font-medium mb-3 md:mb-4 mt-2 md:mt-3 m-0">
                                        {step.title}
                                    </h4>
                                    <p className="text-[#334155] leading-[1.6] m-0 font-normal">
                                        {step.description}
                                    </p>
                                </div>
                            </div>
